...
use App\Models\Attendance; // Jangan lupa import model ini
...
class ExternalDashboard extends Component
{
    ...
    public function refreshData()
    {
        try {
            ...
            // Pastikan status absen juga ikut di-refresh
    ...
    public function mount($token)
    {
        ...
        if (!Session::get('external_authenticated_' . $this->token)) {
        ...
        $this->loadWidgetData();
        // Cek apakah user yang login sudah absen hari ini
        $this->checkAttendanceStatus();
    }

    public function checkAttendanceStatus() {
        if (auth()->check()) {
            $this->hasClockedIn = Attendance::where('user_id', auth()->id())
                ->where('date', now()->format('Y-m-d'))
                ->exists();
        } else {
            // Jika tidak login, kita anggap false (atau true buat ngetes)
            $this->hasClockedIn = false;
            
            // Uncomment baris bawah ini untuk melihat apakah tampilan berubah kalau user dianggap 'hadir'
